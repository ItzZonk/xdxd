    ...
    stmt = insert(Schedule).values([
        {
            ...
            "room_number": l.room,
            "start_time": time.fromisoformat(l.start_time) if l.start_time else None,
            "end_time": time.fromisoformat(l.end_time) if l.end_time else None,
            "group_name": l.group_name or None,
...
async def notify_subscribers(session: AsyncSession, bot):
    """
    Sends notification to subscribed users (Optimized).
    """
    import asyncio
    ...
    logging.info(f"Sending notifications to {len(users)} users in batches of {batch_size}...")
    
    async def send_safe(user_id):
        try:
            await bot.send_message(user_id, "🔔 Расписание обновилось! Проверьте изменения.")
            return 1
        except Exception:
            return 0
    ...
    for i in range(0, len(users), batch_size):
        ...
        tasks = [send_safe(u.telegram_id) for u in batch]

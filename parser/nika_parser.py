...
class LessonRaw:
    ...
    end_time: str = ""     # "08:40"
    group_name: str = ""   # "1 группа" or empty for whole class
...
class NikaParser:
    ...
    def normalize_schedule(self, raw_data: Dict[str, Any]) -> List[LessonRaw]:
        ...
        Now includes lesson times and handles group splits.
        """
        ...
        lesson_times = raw_data.get("LESSON_TIMES", {})
        class_groups = raw_data.get("CLASSGROUPS", {})
        ...
        for class_id, class_schedule in full_schedule.items():
            ...
            for slot_key, slot_data in class_schedule.items():
                ...
                group_ids = slot_data.get("g", [])  # Group IDs if class is split
                
                # Check if this slot has groups (class is split)
                if group_ids and len(group_ids) > 1:
                    # Create separate lessons for each group
                    for idx, group_id in enumerate(group_ids):
                        group_name = class_groups.get(str(group_id), f"Группа {idx+1}")
                        
                        # Get values at this index, fallback to first if not enough
                        subj = subjects.get(sub_ids[idx] if idx < len(sub_ids) else sub_ids[0], "") if sub_ids else ""
                        teach = teachers.get(teach_ids[idx] if idx < len(teach_ids) else teach_ids[0], "") if teach_ids else ""
                        room = rooms.get(room_ids[idx] if idx < len(room_ids) else room_ids[0], "") if room_ids else ""
                    ...
                    # No groups - single lesson for whole class
